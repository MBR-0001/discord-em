import Discord, { CacheType, GuildEmoji, Interaction } from "discord.js";
...
client.on("interactionCreate", (interaction: Interaction<CacheType>) => {
    ...
    if (!opt) return error("No option found");
    ...
    if (interaction.isAutocomplete()) {
        ...
        const f = (x: GuildEmoji) => x.name != undefined && (x.name && x.animated || x.guild.id != interaction.guild!.id) && x.name.toLowerCase().includes(val.toLowerCase());
        const e = [...interaction.client.emojis.cache.values()].filter(f).slice(0, 25);

        return interaction.respond(e.map(x => ({name: x.name as string, value: x.id})));
    }

    const e = interaction.client.emojis.cache.get(val) ?? interaction.client.emojis.cache.find(x => x.name == val);
    if (!e) return error("Invalid emoji provided");
